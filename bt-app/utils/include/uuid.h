...
inline std::ostream& operator<<(std::ostream& os, const btapp::Uuid& a) {
  os << a.ToString();
  return os;
}

// Custom std::hash specialization so that btapp::UUID can be used as a key
// in std::unordered_map.
namespace std {

template <>
struct hash<btapp::Uuid> {
  std::size_t operator()(const btapp::Uuid& key) const {
