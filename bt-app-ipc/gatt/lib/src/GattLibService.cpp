...
void GattLibService::HardcodeFeatureSet (void)
{
	...
	mNumOfOffloadedIrkSupported = 32;                  
	mNumOfOffloadedScanFilterSupported = 16;                  
	mOffloadedScanResultStorageBytes = 10240;                  
	mIsActivityAndEnergyReporting = 1;                  
	mVersSupported = 97;                  
	mTotNumOfTrackableAdv = 32;                  
	mIsExtendedScanSupported = 1;                  
	mIsDebugLogSupported = 0;                  
	mIsLe2MPhySupported = 1;                  
	mIsLeCodedPhySupported = 1;                  
	mIsLeExtendedAdvertisingSupported = 1;                  
	mIsLePeriodicAdvertisingSupported = 1;                  
	mLeMaximumAdvertisingDataLength = 1650;

  ALOGD(LOGTAG "BT_PROPERTY_LOCAL_LE_FEATURES (update by default)\n \
...
void GattLibService::HandleBleScannerTrackAdvEvent(BleScannerTrackAdvEvent *event)
{
  ...
  delete [] event->p_adv_track_info.p_adv_pkt_data;
  delete [] event->p_adv_track_info.p_scan_rsp_data;
  delete event->p_adv_track_info.bd_addr;
  delete adv_info;
