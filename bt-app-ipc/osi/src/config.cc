...
bool config_save(const config_t *config, const char *filename) {
  ...
  int temp_filename_size = strlen(filename) + 5;
  ...
  strlcpy(temp_filename, filename, temp_filename_size);
  strlcat(temp_filename, ".new", temp_filename_size);
...
static void config_parse(FILE *fp, config_t *config) {
  ...
  while (fgets(line, sizeof(line), fp)) {
    ...
    if (*line_ptr == '[') {
      ...
      strlcpy(section, line_ptr + 1, len - 2 + 1);
      section[len - 2] = '\0';
    } else {
