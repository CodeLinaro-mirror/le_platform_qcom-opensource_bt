...
#define LOGTAG "GattNativeInterfaceV2bImpl"
#define LOG_NDEBUG 0
...
#define READ_SDBUS_ARRAY_TO_VECTOR( m, datatype, sdbus_type, vect_name )  { \
  ...
  for(;;) { \
      ...
      if( 0 == res || res < 0 ) break;  \
      ALOGE(LOGTAG "%s Read data from sdbus array : %d", __func__, vect);  \
...
#define WRITE_SDBUS_ARRAY_FROM_VECTOR( m, datatype, sdbus_type, vect_name )  { \
  ...
  for(const auto it : vect_name)  { \
      ...
      if( res < 0 ) break;  \
      ALOGD(LOGTAG "%s Write data to sdbus array : %d", __func__, it);  \
  } \
...
uint8_t status;
////////////////////////////
  ALOGD(LOGTAG "(%s) enter", __FUNCTION__);
  const char *str_get;
  bool is_valid;
  int rtn;
...
uint16_t event_type; uint8_t addr_type;
...
////////////////////////////
 ...
 if( rtn < 0 ) {
   ALOGE(LOGTAG "::%s Failed to read parameters: %d - %s\n", __func__, -rtn, strerror(-rtn));
    ...
    {
      ALOGD(LOGTAG "(%s) status : %d client_if : %d",__FUNCTION__, client_if, status, report_format, num_records, data);
      if(sGattScannerCallbacks) 
...
int status; int clientIf; bluetooth::Uuid app_uuid;	
////////////////////////////
  ALOGD(LOGTAG "(%s) enter", __FUNCTION__);
  bool is_valid;
  const char *str_get;
  ...
  if( rtn < 0 ) {
	...
	if (is_valid)
	{
	    ALOGD(LOGTAG " (%s) status = %d, clientIf = %d, app_uuid = %s", __FUNCTION__, status, clientIf, app_uuid.ToString().c_str());
 ...
 int conn_id; int status; int clientIf; RawAddress bda;
 ////////////////////////////
 ALOGD(LOGTAG "(%s) enter", __FUNCTION__);
 const char *str_get;
 int rtn;
 ...
 if( rtn < 0 ) {
   ALOGE(LOGTAG "::%s Failed to read parameters: %d - %s\n", __func__, -rtn, strerror(-rtn));
	 ...
	 bda = str2addr(string(str_get));
   ALOGD(LOGTAG "(%s) conn_id: %d status: %d client_if: %d", __FUNCTION__, conn_id, status, clientIf);
   if(sGattClientCallbacks) 
...
int conn_id; int registered; int status; uint16_t handle;	
  ...
  if( rtn < 0 ) {
    ...
    ALOGD(LOGTAG "(%s) conn_id: %d status: %d registered: %d, handle: %d", __FUNCTION__, conn_id, status, registered, handle);
    if(sGattClientCallbacks) 
...
static int btgattc_notify_cb(sd_bus_message *m, void *userdata, sd_bus_error *ret_error) {
  ...
  ALOGD(LOGTAG "(%s) conn_id : %d, len : %d",__FUNCTION__,conn_id, p_data.len);
    if(sGattClientCallbacks) 
...
int client_if; RawAddress bda; int rssi; int status;	
  ////////////////////////////
  ALOGD(LOGTAG "(%s) enter", __FUNCTION__);
  const char *str_get;
  int rtn;
  ...
  if( rtn < 0 ) {
	...
	bda = str2addr(string(str_get));
    ALOGD(LOGTAG "(%s) status : %d client_if : %d, rssi: %d", __FUNCTION__,status, client_if, rssi);
    if(sGattClientCallbacks) 
...
int conn_id; int status; int mtu;	
  ...
  if( rtn < 0 ) {
    ...
    ALOGD(LOGTAG "(%s) status : %d conn_id : %d, mtu: %d", __FUNCTION__, status, conn_id, mtu);
    if(sGattClientCallbacks) 
...
static int btgattc_get_gatt_db_cb(sd_bus_message *m, void *userdata, sd_bus_error *ret_error) {
  int conn_id;
  btgatt_db_element_t db={0,};
  //const btgatt_db_element_t* db;
  int count;	

  /* signature : iqsiqqqyqi  */
  ALOGD(LOGTAG "(%s) enter", __FUNCTION__);
  ...
  rtn = sd_bus_message_read( m, "qsiqqqyq", &db.id,
                                            &str_get,
                                            &type,
                                            &db.attribute_handle,
                                            &db.start_handle,
  ...
  if( is_valid ) db.uuid = btAppUuid2bluetoothUuid(uuid);
  db.type = (bt_gatt_db_attribute_type_t)type;

  rtn = sd_bus_message_read( m, "i", &count);
  if( rtn < 0 ) {
    ALOGE(LOGTAG "::%s Failed to read parameters: %d - %s\n", __func__, -rtn, strerror(-rtn));
    return sd_bus_reply_method_return(m, nullptr);
  }
...
int conn_id; uint8_t tx_phy; uint8_t rx_phy; uint8_t status;	
  ////////////////////////////
  ALOGD(LOGTAG "(%s) enter", __FUNCTION__);
  int rtn;
  ...
  if( rtn < 0 ) {
    ...
    ALOGD(LOGTAG "(%s) conn_id: %d tx_phy: %d, rx_phy: %d, status: %d", __FUNCTION__,conn_id, tx_phy, rx_phy, status);
    if(sGattClientCallbacks) 
...
int conn_id; uint16_t interval; uint16_t latency; uint16_t timeout; uint8_t status;	
  ////////////////////////////
  ALOGD(LOGTAG "(%s) enter", __FUNCTION__);
  int rtn;
  ...
  if( rtn < 0 ) {
    ...
    ALOGD(LOGTAG "(%s) conn_id: %d interval: %d, latency: %d, timeout: %d status: %d", __FUNCTION__,conn_id, interval, latency, timeout, status);
    if(sGattClientCallbacks) 
...
uint8_t clientIf; RawAddress bda; uint8_t tx_phy; uint8_t rx_phy; uint8_t status;
  ////////////////////////////
  ALOGD(LOGTAG "(%s) enter", __FUNCTION__);
  const char *str_get;
  int rtn;
  ...
  if( rtn < 0 ) {
    ...
    ALOGD(LOGTAG "(%s) clientIf: %d, bda: %s, tx_phy: %d, rx_phy: %d, status: %d",
        __FUNCTION__,clientIf,  addr2Str(bda)->c_str(), tx_phy, rx_phy, status);
    if(sGattClientCallbacks) 
...
std::vector<btgatt_db_element_t> service;
  ...
  for(;;) {
    ...
    rtn = sd_bus_message_read( m, "qsiqqqyq", &data.id,
                                              ...
                                              &data.properties,
                                              &data.permissions);
    if( 0 == rtn || rtn < 0 ) break;
    
    uuid = btapp::Uuid::FromString(str_get, &is_valid);
    if( is_valid ) data.uuid = btAppUuid2bluetoothUuid(uuid);
  ...
  ALOGD(LOGTAG "(%s) status : %d server_if : %d, service size:%d",__FUNCTION__, status, server_if, service.size());
  if(sGattServerCallbacks){
...
ALOGD(LOGTAG "(%s) enter", __FUNCTION__);
  ...
  if( rtn < 0 ) {
    ...
    ALOGD(LOGTAG "(%s) connid: %d trans_id: %d, bda: %s, attr_handle: %d, offset:%d is_long: %d",
        __FUNCTION__, conn_id, trans_id, addr2Str(bda)->c_str(), attr_handle, offset, is_long);
    if(sGattServerCallbacks){
      sGattServerCallbacks->request_read_characteristic_cb (conn_id, trans_id, bda, attr_handle, offset, is_long);}
  	}
  return sd_bus_reply_method_return(m, nullptr);  
}
...
ALOGD(LOGTAG "(%s) enter", __FUNCTION__);
  ...
  if( rtn < 0 ) {
    ...
    if(sGattServerCallbacks){
      sGattServerCallbacks->request_read_descriptor_cb (conn_id, trans_id, bda, attr_handle, offset, is_long);}
  }
  return sd_bus_reply_method_return(m, nullptr);  
}
...
ALOGD(LOGTAG "(%s) enter", __FUNCTION__);
  ...
  if( rtn < 0 ) {
   ...
   ALOGD(LOGTAG "(%s) connid: %d trans_id: %d, bda: %s, attr_handle: %d, offset:%d need_rsp: %d,"
      " is_prep:%d", __FUNCTION__, conn_id, trans_id, addr2Str(bda)->c_str(), attr_handle, offset,
      need_rsp, is_prep);
   if(sGattServerCallbacks){
     sGattServerCallbacks->request_write_characteristic_cb ( conn_id, trans_id, bda, attr_handle, offset, need_rsp, is_prep, value);}
  }
  return sd_bus_reply_method_return(m, nullptr);  
}
...
////////////////////////////
  ...
  if( rtn < 0 ) {
    ...
    if(sGattServerCallbacks){
     sGattServerCallbacks->request_write_descriptor_cb (conn_id, trans_id, bda, attr_handle, offset, need_rsp, is_prep, value);}
  }
  return sd_bus_reply_method_return(m, nullptr);  
}
...
int exec_write;
  ...
  if( rtn < 0 ) {
    ...
    ALOGD(LOGTAG "(%s) connid: %d trans_id: %d, bda: %s, exec_write: %d",
        __FUNCTION__, conn_id, trans_id, addr2Str(bda)->c_str(), exec_write);
...
int congested; // bool	
  ...
  if( rtn < 0 ) {
    ...
    if(sGattServerCallbacks){
      sGattServerCallbacks->congestion_cb (conn_id, congested);}
  }
  return sd_bus_reply_method_return(m, nullptr);  
}
...
uint8_t serverIf; RawAddress bda; uint8_t tx_phy;
uint8_t rx_phy; uint8_t status;
  ...
  if( rtn < 0 ) {
    ...
    ALOGD(LOGTAG "(%s) serverIf: %d, bda: %s, tx_phy: %d, rx_phy: %d, status: %d",
        __FUNCTION__,serverIf,  addr2Str(bda)->c_str(), tx_phy, rx_phy, status);
...
uint8_t client_if; uint8_t avbl_space; uint8_t action;
uint8_t status;	
  ////////////////////////////
  ALOGD(LOGTAG "(%s) enter", __FUNCTION__);
  int rtn;
...
uint8_t client_if; uint8_t action; uint8_t status;	
  ////////////////////////////
  ALOGD(LOGTAG "(%s) enter", __FUNCTION__);
  int rtn;
...
uint8_t client_if; uint8_t status;	
  ////////////////////////////
  ALOGD(LOGTAG "(%s) enter", __FUNCTION__);
  int rtn;
  rtn = sd_bus_message_read( m, "yy", &client_if, &status);
...
RawAddress address;	
  ...
  if( rtn < 0 ) {
    ...
    ALOGD(LOGTAG "(%s) advertiser_id: %d address_type: %d, address: %s",
        __FUNCTION__, advertiser_id, address_type, addr2Str(address)->c_str());
...
uint8_t status;	
  ...
  if(sGattExtCallbacks){
    sGattExtCallbacks->ble_advertising_set_started_cb( reg_id, advertiser_id, tx_power, status);}
  }
  return sd_bus_reply_method_return(m, nullptr);  
}
...
uint8_t status;	
  ...
  if(sGattExtCallbacks){
    sGattExtCallbacks->ble_advertising_set_enable_Cb( advertiser_id, enable, status);}
  }
  return sd_bus_reply_method_return(m, nullptr);  
}
static int ble_advertising_parameters_updated_cb(sd_bus_message *m, void *userdata, sd_bus_error *ret_error) {
uint8_t advertiser_id; uint8_t status; int tx_power;	
  ////////////////////////////
  ALOGD(LOGTAG "(%s) enter", __FUNCTION__);
  int rtn;
...
uint8_t status;	
  ...
  if(sGattExtCallbacks){
    sGattExtCallbacks->ble_periodic_advertising_set_enable_Cb( advertiser_id, enable, status);}
  }
  return sd_bus_reply_method_return(m, nullptr);  
}
/**
...
void GattNativeInterfaceV2bImpl:: init (const btgatt_native_interface_callbacks_t *btgatt_native_interface_callbacks_a)
{
	...
	{
		ALOGD(LOGTAG "(%s) set callbacks",__FUNCTION__);
		...
		{
			ALOGD(LOGTAG "(%s) set callbacks - scanner",__FUNCTION__);
			sGattScannerCallbacks = btgatt_native_interface_callbacks->btgatt_callbacks->scanner;
			ALOGD(LOGTAG "(%s) set callbacks - client",__FUNCTION__);
			sGattClientCallbacks = btgatt_native_interface_callbacks->btgatt_callbacks->client;
			ALOGD(LOGTAG "(%s) set callbacks - server",__FUNCTION__);
			sGattServerCallbacks = btgatt_native_interface_callbacks->btgatt_callbacks->server;
		}
		ALOGD(LOGTAG "(%s) set callbacks - extension",__FUNCTION__);
		sGattExtCallbacks = btgatt_native_interface_callbacks->extension; 
	}

// initialize dbus
	if( !open_bus() ) {
	  ALOGE(LOGTAG "(%s) Failed to open sd-bus",__FUNCTION__);
	  return;
	}

	if( !initSDBus() ) {
	  ALOGE(LOGTAG "(%s) Failed to init sd-bus",__FUNCTION__);
	  return;
	}
	ALOGD(LOGTAG "(%s) Success to setup sd-bus",__FUNCTION__);
...
void GattNativeInterfaceV2bImpl::deinit (void)
{
	...
	ALOGD(LOGTAG "(%s) Success to cleanup sd-bus",__FUNCTION__);
}

#define params_not_defined_yet__ "i"
bool GattNativeInterfaceV2bImpl::initSDBus (void)
{
    ...
    static const sd_bus_vtable dbus_vtable[] = {
	  ...
	  SD_BUS_METHOD("btgattc_get_gatt_db_cb", "iqsiqqqyqi", nullptr, btgattc_get_gatt_db_cb, SD_BUS_VTABLE_UNPRIVILEGED),
...
const char * __gatt_ipc_service = "com.qualcomm.bt";
const char * __gatt_ipc_service_object = "/com/qualcomm/bt/gatt";
const char * __gatt_ipc_service_name = "com.qualcomm.bt.gatt";
#define __gatt_message_release(rtn, m) {\
	  if( rtn < 0 ) { \
		ALOGE(LOGTAG "(%s) Failed to call method(%s) : %d - %s", __func__, -rtn, strerror(-rtn)); \
	  } \
...
int GattNativeInterfaceV2bImpl :: gattClientGetDeviceTypeNative(string address) {
  ...
  int rtn, result=-1;
  rtn = sd_bus_call_method(__primary_params, __FUNCTION__,
  ...
  if ( rtn < 0 )  {
    ALOGD(LOGTAG "(%s) rtn:%d",__FUNCTION__, rtn);
    goto finish;
  }
...
void GattNativeInterfaceV2bImpl :: registerScannerNative(btapp::Uuid uuid) {
 ...
 ALOGD(LOGTAG "(%s) enter!, uuid: %s",__FUNCTION__, uuid.ToString().c_str());
 // implement IPC call here by passing the parameters unchanged
 sd_bus_message *m = NULL;
 int rtn;
 rtn = sd_bus_call_method(__primary_params, __FUNCTION__,
...
void GattNativeInterfaceV2bImpl :: gattClientConnectNative(int clientif, string address, bool isDirect,
  ...
  rtn = sd_bus_call_method(__primary_params, __FUNCTION__,
     ...
     clientif, address.c_str(), isDirect, transport, opportunistic, initiating_phys ); 		  /* parameters */ 
...
void GattNativeInterfaceV2bImpl :: gattClientSearchServiceNative(int conn_id, bool search_all,
  ...
  rtn = sd_bus_call_method(__primary_params, __FUNCTION__,
     ...
     &m,							   /* return message on success */
     "iis",			   /* input signature */
     conn_id, search_all, uuid.ToString().c_str());	   /* parameters */ 
...
void GattNativeInterfaceV2bImpl :: gattClientWriteCharacteristicNative(int conn_id, int handle,
  ...
  rtn = sd_bus_call(g_sdbus, m, -1, nullptr, nullptr);
  if( rtn < 0 ) {
...
void GattNativeInterfaceV2bImpl :: gattClientRegisterForNotificationsNative(
  ...
  rtn = sd_bus_call_method(__primary_params, __FUNCTION__,
     ...
     clientIf, address.c_str(), handle, enable); /* parameters */ 
...
void GattNativeInterfaceV2bImpl ::  gattServerRegisterAppNative(btapp::Uuid uuid) {
  ...
  if( rtn < 0 )
    ALOGD(LOGTAG "(%s) send uuid:%s, rtn:%d - %s",__FUNCTION__, uuid.ToString().c_str(), -rtn, strerror(-rtn));
  else
    ALOGD(LOGTAG "(%s) send uuid:%s, rtn:%d",__FUNCTION__,uuid.ToString().c_str(), rtn);
...
void GattNativeInterfaceV2bImpl :: gattServerConnectNative(int server_if,
	...
	rtn = sd_bus_call_method(__primary_params, __FUNCTION__,
			   ...
			   &m,						  /* return message on success */
			   "isii", 					  /* input signature */
			   server_if, address.c_str(), is_direct, transport );		  /* parameters */ 
...
void GattNativeInterfaceV2bImpl :: gattServerAddServiceNative(int server_if,std::vector<gatt_db_element_t> service) {
	...
	ALOGD(LOGTAG "(%s) enter : service size - %d",__FUNCTION__, service.size());
  ...
  for(const auto it : service)  { 
    ...
    if( rtn < 0 ) {
	  ...
	  ALOGD(LOGTAG "(%s) id:%d, uuid:%s",__FUNCTION__, it.id, it.uuid.ToString().c_str());
...
void GattNativeInterfaceV2bImpl :: enableAdvertisingSetNative(
	...
	rtn = sd_bus_call_method(__primary_params, __FUNCTION__,
			   ...
			   &m,						  /* return message on success */
			   "iiii", 				  /* input signature */
			   advertiser_id, enable, duration, maxExtAdvEvents);	  /* parameters */ 
...
void GattNativeInterfaceV2bImpl :: setPeriodicAdvertisingEnableNative(
  ...
  rtn = sd_bus_call_method(__primary_params, __FUNCTION__,
        ...
        &m,						  /* return message on success */
        "ii",				  /* input signature */
        advertiser_id, enable);	  /* parameters */ 
...
static void btgatt_event_cb (BtEvent *event)
{
	switch(event->event_id) {
		...
		case RSP_DISABLE_EVENT:
			break;
		default:
			ALOGD(LOGTAG "(%s) no event id (%x)", __FUNCTION__, event->event_id);
			break;
	};
}
*/
